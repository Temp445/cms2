...
const PricingTable: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-8 sm:px-6 md:px-4 py-10 md:py-16" id="pricing">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-2">
        {plans.map(plan => (
          <div
            key={plan.name}
            className={`relative bg-white rounded border ${plan.highlighted ? 'border-teal-600 shadow-lg' : 'border-gray-200 shadow'} hover:shadow-xl transition-all`}
          >
            {plan.tag && (
              <div className="absolute top-5 right-0">
                <span className="bg-teal-500 text-white px-3 py-1 rounded-l-full text-sm font-medium shadow-sm">
                  {plan.tag}
                </span>
              </div>
            )}
            <div className="p-5 lg:p-8">
             ...
             <label className="text-sm text-gray-600 block mb-1">
              ...
              <button
                className="w-full md:hidden bg-black text-white py-2 rounded-sm font-semibold flex items-center justify-center mt-8 mb-4"
